...
var OrderService = (function (_super) {
    ...
    function OrderService(http, _httpSettings) {
        _super.call(this, http, _httpSettings);
        this.http = http;
        this._httpSettings = _httpSettings;
        this._basePath = 'orders/';
    }
    OrderService.prototype.listMap = function (res) {
        var toReturn = res.json();
        for (var num in toReturn.results) {
            if (toReturn.results.hasOwnProperty(num)) {
                toReturn.results[num] = new order_1.Order(toReturn.results[num]);
            }
        }
        return toReturn;
    };
    OrderService.prototype.singleMap = function (res) {
        return new order_1.Order(res.json());
    };
