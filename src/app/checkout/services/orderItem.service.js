...
var ProductService = (function (_super) {
    ...
    function ProductService(http, _httpSettings) {
        _super.call(this, http, _httpSettings);
        this.http = http;
        this._httpSettings = _httpSettings;
        this._basePath = 'order/items/';
    }
    ProductService.prototype.listMap = function (res) {
        var toReturn = res.json();
        for (var num in toReturn.results) {
            if (toReturn.results.hasOwnProperty(num)) {
                toReturn.results[num] = new order_item_1.OrderItem(toReturn.results[num]);
            }
        }
        return toReturn;
    };
    ProductService.prototype.singleMap = function (res) {
        return new order_item_1.OrderItem(res.json());
    };
